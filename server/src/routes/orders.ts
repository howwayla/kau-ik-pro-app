...
export function registerOrderRoutes(
    ...
    app.post<{ Body: PlaceOrderBody }>(
        ...
        async (req, reply) => {
            const { contract, stock_order, futures_order } = req.body;
            if (stock_order) {
                return ctx.trading.placeStockOrder(contract, stock_order);
            }
            if (futures_order) {
                if (!ctx.trading.capabilities().futures) {
                    return reply.code(400).send({
                        detail: '此券商不支援期貨/選擇權下單',
                    });
                }
                return ctx.trading.placeFuturesOrder(contract, futures_order);
            }
            return reply
                .code(400)
